...
def in_bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
...
def in_box_iou(box1, box2, eps=1e-7):
...
def segments2boxes(segments):
    ...
    return in_xyxy2xywh(np.array(boxes))  # cls, xywh
...
def ap_per_class_box_and_mask(
    ...
    """
    results_boxes = in_ap_per_class(tp_b,
                                 conf,
                                 pred_cls,
                                 target_cls,
                                 plot=plot,
                                 ...
                                 names=names,
                                 prefix="Box")[2:]
    results_masks = in_ap_per_class(tp_m,
                                 conf,
                                 pred_cls,
                                 target_cls,
                                 plot=plot,
...
def in_ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16, prefix=""):
...
def in_output_to_target(output, max_det=300):
    # Convert model output to target format [batch_id, class_id, x, y, w, h, conf] for plotting
    targets = []
    for i, o in enumerate(output):
        box, conf, cls = o[:max_det, :6].cpu().split((4, 1, 1), 1)
        j = torch.full((conf.shape[0], 1), i)
        targets.append(torch.cat((j, cls, in_xyxy2xywh(box), conf), 1))
